...
images_folder_path='assets/img/post'
...
for exported_foldername in ${exported_foldername_array[*]}; do
    ...
    sed -i "" -e $'5 a\\\n'"date: $(date +%Y)-$(date +%m)-$(date +%d)" "$exported_file_path"
    sed -i "" -e $'6 a\\\n'"excerpt: \"$meta_excerpt\"" "$exported_file_path"
    sed -i "" -e $'7 a\\\n'"comments: true" "$exported_file_path"
    sed -i "" -e $'8 a\\\n'"---" "$exported_file_path"


    # Making a post file name
    fixed_filename="$(date +%Y)-$(date +%m)-$(date +%d)-$meta_title"
    ...
    sed -i '' "s|$exported_filename_for_images_path|/$images_folder_path/$fixed_filename|g" "$exported_file_path"
    perl -0777 -pi -e 's|(?<=---\n\n)(.*?)(?=#)||igs' "$exported_file_path" # md파일 설정부분부터 내용이 시작하는 부분까지 작성되어있는 Notion 기본 생성 내용 삭제
